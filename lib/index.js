...
// Get list of files defined in the dsc
DscGrabber.prototype.getFile = function(index) {
  var self = this;
  return new Promise(function(fulfill, reject) {
    self.files().then(function(list) {
      if (index > list.length) {
        reject({code: "ENOENT"});
      } else {
        var dirIndex = self.file.lastIndexOf("/");
        var path = self.file.substr(0, dirIndex);
        var f = fs.createReadStream(path + "/" + list[index].name); 
        fulfill({meta: list[index], stream: f});
      }
    }).catch(function(err) {
      reject(err);
    });
